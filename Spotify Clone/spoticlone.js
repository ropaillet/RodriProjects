...
function showData(dataArtista,cantidadArtistas){
	...
	for (var i = 0 ; i < cantidadArtistas; i++) {
		...
		$(".resultsArea").append('<div class="col-sm-2">'+
					...
					"<h4 class='albumName'><i class='fa fa-star hidden' id='"+dataArtista.artists.items[i].id+"Full'></i><i class='fa fa-star-o' id='" + dataArtista.artists.items[i].id +"'></i><span class='artistName'>"+ " " + dataArtista.artists.items[i].name+
					"</span></h4></div>"
		)
	}
}
